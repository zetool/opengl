...
/**
 * A utility class that allows to set up several projections in the {@code OpenGL} world. Whenever a method (except
 * {@link #resetProjection(javax.media.opengl.GL)}) is called, the current projection and modelview matrices are pushed
 * to the stack.
 *
...
public final class ProjectionHelper {
     ...
     */
    public static void setViewOrthogonal(GL2 gl, int viewportWidth, double viewportHeight, double x, double y, double z, double favouredWidth, double favouredHeight, double depth) {
    ...
    /**
     * Sets up an orthognal projection. In opposite to {@link #setViewOrthogonal()}, the visible range is stretched if
     * the screen is wider than the viewport. In that case the upper and lower part are cut of.
     *
     * @param gl the OpenGL graphics context
     ...
     * @param depth the depth that can be used to draw
     */
    public static void setViewStretchedOrthogonal(GL2 gl, int viewportWidth, double viewportHeight, double x, double y, double z, double favouredWidth, double favouredHeight, double depth) {
        // Avoid division by zero
        if (viewportHeight <= 0) {
            viewportHeight = 1;
        }
     ...
     * @param twist
     */
    public static void setViewPolar(GL2 gl, double distance, double azimuth, double incidence, double twist) {
    ...
    /**
     * Sets up a projection to print text on the screen. It uses an orthogonal projection and allows to address each
     * pixel in the submittet range.
     *
     * @param gl the OpenGL render context
     * @param width the width of the screen
     * @param height the height of the screen
     */
    public static void setPrintScreenProjection(GL2 gl, int width, int height) {
    ...
    /**
     * Resets a projection to the old status by deleting projection matrices from the stack.
     *
     * @param gl the OpenGL render context
     */
    public static void resetProjection(GL2 gl) {
        // restore matrices
        gl.glMatrixMode(GL2.GL_PROJECTION);
        gl.glPopMatrix();
        gl.glMatrixMode(GL2.GL_MODELVIEW);
