...
/**
 * The class {@code GLColor} encapsulates a <i>color</i> defined by RGB values. Each value (red, green and blue) can be
 * in the range from 0.0 to 1.0 or from 0 to 255. Additionally each color can have an alpha value, where a value of 0
 * means that the color is transparent. The class provides methods to set it in {@code OpenGL} as a color. It also
 * allows blending and additional operations.
 *
...
public final class GLColor implements Drawable {
    ...
    /**
     * Initializes a color with additional transparency. All parameters are float which is the natural floating point
     * precision of {@code OpenGL}.
     *
    ...
    /**
     * Initialize with java {@code Color} object without any alpha. The alpha of the color is ignored.
     *
     * @param c the color
     */
    ...
    /**
     * Initialize with java {@code Color} object and specified alpha value that differs from the colors alpha.
     *
     * @param c the color
     ...
     */
    private void setValue(double r, double g, double b, double a) {
        R = r;
        G = g;
        B = b;
        A = a;
    ...
    /**
     * Corrects color values. Values larger than 1.0 are reduced to 1.0 and values smaller than 0.0 are increased to
     * 0.0.
     */
    ...
    /**
     * Sets the color in {@code OpenGL}, can set color using materials and color depending from the current mode.
     * 
     * Note that it is not allowed to use {@code glIsEnabled()} between {@code glBegin()} and {@code glEnd()}. The
     * method is used here, so to change the color in that special case use {@link #performGL( GL, boolean )}
     * instead.
     *
    ...
    public void draw(GL2 gl) {
        if (gl.glIsEnabled(GL2.GL_LIGHTING) && !gl.glIsEnabled(GL2.GL_COLOR_MATERIAL)) {
    ...
    /**
     * Sets the color in {@code OpenGL}, can set color using materials and color, depending from the value of
     * {@code lighting}.
     *
     * @param gl the {@code OpenGL} context on which shall be drawn
     * @param lighting sets whether the lighting or non-lighting version should be used
     */
    public final void draw(GL2 gl, boolean lighting) {
        if (lighting) {
            gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_DIFFUSE, mat, 0);
            gl.glMaterialfv(GL2.GL_FRONT, GL2.GL_AMBIENT, mat, 0);
        } else {
    ...
    /**
     * Returns a new {@code GLColor}. The {@code percentage} how bright the color will be. A value of 0 returns black
     * while a value of 1 will a copy of this instance.
     *
    ...
    /**
     * Returns a 50%-mixture of the passed {@code color} and the color of this instance.
     *
    ...
    /**
     * Creates a blended color that is a mixture of {@code percentage} fraction of this color and {@code 1-percentage}
     * fraction of {@code color}. A percentage of 1 will return a copy of {@code color}, a value of 0 will return a copy
     * of this instance.
     *
    ...
    /**
     * Returns a new {@code GLColor}. The {@code percentage} how bright the color will be. A value of 0 returns a copy
     * of this instance white while a value of 1 will return white.
     *
     * @param percentage the brightness of the color.
     * @return the created blended color
     */
